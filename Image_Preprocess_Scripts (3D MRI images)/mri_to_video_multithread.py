...
# I tried performing the MRI_TO_VIDEO operation with multiple threads, but that had no benefit over doing them sequentially
...
def process_nii_file(nii_file_name):
...
nii_files = os.listdir(nii_images_path)

with ThreadPoolExecutor() as executor:
    futures = [executor.submit(process_nii_file, f) for f in nii_files]
    for _ in tqdm(as_completed(futures), total=len(futures), desc='Preprocessing images'):
        pass